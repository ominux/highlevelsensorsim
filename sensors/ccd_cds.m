...
%> Correlated Double Sampling (CDS) is a technique for measuring photovoltage
...
%> light signal read by two circuits.@n
%> 
...
%> fixed-pattern noise can be eliminated.@n
%> 
%> CDS is commonly used in image sensors to reduce FPN and reset noise. CDS only
%> reduces offset FPN (gain FPN cannot be reduced using CDS). CDS in  in CCDs, PPS,
